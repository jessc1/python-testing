...
class CartWithOneItem(unittest.TestCase):
    def setUp(self):
        self.cart = ShoppingCart().add("mushroom sandwich", 15.00)
    ...
    def test_when_looking_into_cart_should_be_one_based(self):
        self.assertEquals("mushroom sandwich", self.cart.item(1))
...
class CarWithTwoItems(unittest.TestCase):
    def setUp(self):
        self.cart = ShoppingCart() \
                        .add("mushroom sandwich", 15.00) \
                        .add("falafel", 3.75) 
    
    def test_when_checking_the_size_should_be_two(self):
    ...
    def test_items_should_be_in_same_order_as_entered(self):
        ...
        self.assertEquals("falafel", self.cart.item(2))
...
class CartWithNoItems(unittest.TestCase):
    def setUp(self):
        self.cart = ShoppingCart()
    def test_when_checking_the_size_should_be_empty(self):
    ...
    def test_adding_items_returns_back_same_cart(self):
        ...
        cart_with_one_item = self.cart.add("mushroom sandwich", 15.00)
        self.assertEquals(empty_cart, cart_with_one_item)
        cart_with_two_items = self.cart.add("falafel", 3.75)
        self.assertEquals(empty_cart, cart_with_one_item)
